...
VALID_CATEGORIES = ["disease", "pest", "cultural", "equipment", "irrigation", "general"]
...
def _validate_enum(value, allowed, field_name):
    ...
    if value not in allowed:
        raise ValueError(f"Invalid {field_name}: '{value}'. Must be one of {allowed}")
...
def submit_benchmark(user_id, data):
    ...
    with get_db() as conn:
        cursor = conn.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
            (user_id, year, month, area, metric_type, value, region, grass_type, course_type, anonymous),
        )
        benchmark_id = cursor.lastrowid

    logger.info("Benchmark %s submitted by user %s (%s=%s)", benchmark_id, user_id, metric_type, value)
    return benchmark_id


def get_benchmarks(region=None, grass_type=None, course_type=None, metric_type=None, year=None):
...
def create_alert(user_id, data):
    ...
    with get_db() as conn:
        cursor = conn.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
            (user_id, alert_type, title, description, region, state, severity, start_date, end_date),
        )
...
def vote_alert(alert_id, user_id, vote_type):
    ...
    with get_db() as conn:
        ...
        existing = conn.execute(
            "SELECT id, vote_type FROM alert_votes WHERE alert_id = ? AND user_id = ?", (alert_id, user_id)
        ).fetchone()
        ...
        if existing:
            ...
            if old_type == vote_type:
                ...
                row = conn.execute("SELECT upvotes FROM regional_alerts WHERE id = ?", (alert_id,)).fetchone()
...
def share_program(user_id, data):
    ...
    with get_db() as conn:
        cursor = conn.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
            (user_id, title, description, program_type, season, region, grass_type, program_json, is_public),
        )
...
def rate_program(program_id, user_id, rating, comment=None):
    ...
    with get_db() as conn:
        ...
        existing = conn.execute(
            "SELECT id, rating FROM program_ratings WHERE program_id = ? AND user_id = ?", (program_id, user_id)
        ).fetchone()
        ...
        row = conn.execute(
            "SELECT rating_sum, rating_count FROM shared_programs WHERE id = ?", (program_id,)
        ).fetchone()
...
def download_program(program_id, user_id):
    ...
    """
    with get_db() as conn:
        conn.execute("UPDATE shared_programs SET downloads = downloads + 1 WHERE id = ?", (program_id,))
        row = conn.execute("SELECT * FROM shared_programs WHERE id = ?", (program_id,)).fetchone()
...
def get_post_by_id(post_id):
    ...
    """
    with get_db() as conn:
        post_row = conn.execute("SELECT * FROM discussion_posts WHERE id = ?", (post_id,)).fetchone()
...
def create_reply(post_id, user_id, content):
    ...
    with get_db() as conn:
        # Verify post exists
        post = conn.execute("SELECT id FROM discussion_posts WHERE id = ?", (post_id,)).fetchone()
        if not post:
...
def upvote_post(post_id, user_id):
    ...
    """
    with get_db() as conn:
        conn.execute("UPDATE discussion_posts SET upvotes = upvotes + 1 WHERE id = ?", (post_id,))
        row = conn.execute("SELECT upvotes FROM discussion_posts WHERE id = ?", (post_id,)).fetchone()
...
def accept_reply(reply_id, post_id, user_id):
    ...
    with get_db() as conn:
        ...
        post = conn.execute("SELECT user_id FROM discussion_posts WHERE id = ?", (post_id,)).fetchone()

        if not post or int(post["user_id"]) != int(user_id):
            logger.warning(
                "User %s tried to accept reply %s on post %s but is not the author", user_id, reply_id, post_id
            )
            return False

        # Un-accept any previously accepted reply on this post
        conn.execute("UPDATE discussion_replies SET is_accepted = 0 WHERE post_id = ? AND is_accepted = 1", (post_id,))

        # Accept the target reply
        conn.execute("UPDATE discussion_replies SET is_accepted = 1 WHERE id = ? AND post_id = ?", (reply_id, post_id))
...
def delete_post(post_id, user_id):
    ...
    with get_db() as conn:
        row = conn.execute(
            "SELECT id FROM discussion_posts WHERE id = ? AND user_id = ?", (post_id, user_id)
        ).fetchone()
        if not row:
...
def delete_alert(alert_id, user_id):
    ...
    with get_db() as conn:
        row = conn.execute(
            "SELECT id FROM regional_alerts WHERE id = ? AND user_id = ?", (alert_id, user_id)
        ).fetchone()
        if not row:
