...
def register_push_token():
    ...
    try:
        ...
        with _get_db() as conn:
            ...
            conn.execute("UPDATE users SET push_token = ?, push_platform = ? WHERE id = ?", (token, platform, user_id))
    ...
    return jsonify(
        {
            ...
            and (profile.get("primary_grass") is not None or profile.get("greens_grass") is not None),
        }
    )
