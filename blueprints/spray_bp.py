...
from spray_tracker import (
    ...
    get_applications,
    get_monthly_nutrient_breakdown,
...
def api_products_search():
    ...
    results = search_products(
        query, user_id=session["user_id"], category=category, form_type=form_type, inventory_only=inventory_only
    )
...
def api_save_template():
    ...
    tid = save_template(
        session["user_id"], data["name"], data["products"], data.get("application_method"), data.get("notes")
    )
...
def api_moa_check():
    ...
    warnings = []
    for a in apps:
        products = a.get("products_json") or [
            {"product_id": a.get("product_id"), "product_name": a.get("product_name")}
        ]
        for p in products:
...
def api_log_spray():
    ...
    if not is_tank_mix:
        ...
        products_list = [{"product_id": data["product_id"], "rate": data["rate"], "rate_unit": data["rate_unit"]}]
    ...
    for i, p in enumerate(products_list):
        ...
        resolved_products.append({"product": product, "rate": float(p["rate"]), "rate_unit": p["rate_unit"]})
    ...
    mix_result = calculate_tank_mix(
        resolved_products, area_acreage, carrier_gpa, tank_size, tank_count_override=tank_count_from_ui
    )
    ...
    app_data = {
        ...
        if is_multi
        else first.get("product_name", resolved_products[0]["product"]["display_name"]),
...
def api_get_sprays():
    ...
    applications = get_applications(
        user_id, area=area, year=year, start_date=start_date, end_date=end_date, limit=limit
    )
...
def api_spray_csv():
    ...
    def _csv_row(a, product_data=None):
        ...
        return [
            a["date"],
            a["area"],
            a.get("application_method", ""),
            p.get("product_name", ""),
            p.get("product_category", ""),
            p.get("rate", ""),
            p.get("rate_unit", ""),
            p.get("total_product", ""),
            p.get("total_product_unit", ""),
            a.get("carrier_volume_gpa", ""),
            a.get("total_carrier_gallons", ""),
            a.get("weather_temp", ""),
            a.get("weather_wind", ""),
...
def api_spray_pdf_single(app_id):
    ...
    try:
        ...
        return Response(
            ...
            headers={"Content-Disposition": f"attachment; filename=spray_record_{app_id}.pdf"},
        )
    except ImportError:
...
def api_spray_pdf_report():
    ...
    try:
        ...
        pdf_buffer = generate_seasonal_report(applications, nutrient_summary, course_name, date_range=f"Season {year}")
        return Response(
            pdf_buffer.getvalue(),
            mimetype="application/pdf",
            headers={"Content-Disposition": f"attachment; filename=spray_report_{year}.pdf"},
        )
    except ImportError:
        return jsonify({"error": "PDF generation not available. Install reportlab."}), 500
    except Exception as e:
...
def api_spray_pdf_nutrients():
    ...
    try:
        ...
        return Response(
            pdf_buffer.getvalue(),
            mimetype="application/pdf",
            headers={"Content-Disposition": f"attachment; filename=nutrient_report_{year}.pdf"},
        )
    except ImportError:
        return jsonify({"error": "PDF generation not available. Install reportlab."}), 500
    except Exception as e:
