...
from intelligence_engine import IntelligenceScheduler, process_feedback_intelligence
from logging_config import logger
from routes import turf_bp
from weather_service import format_weather_for_response, get_weather_data, get_weather_warnings
                    ...
                    for filename in files:
                        if filename.lower().endswith(".pdf") and not filename.startswith("."):
                            ...
                            resources_list.append(
                                {"filename": filename, "url": f"/static/{relative_path}", "category": category}
                            )
...
def submit_feedback():
    try:
        ...
        update_query_rating(question=question, rating=rating, correction=correction or None)
        ...
        try:
            from db import FEEDBACK_DB as _fb_db
        ...
        if rating and str(rating) in ("-1", "0", "bad", "negative") and question:
            try:
                ...
                logger.debug("Cache invalidated for question after negative feedback")
            except Exception as cache_err:
                logger.warning(f"Cache invalidation failed: {cache_err}")

        # Community knowledge loop — log corrections and knowledge gaps
        try:
            from turf_intelligence import log_knowledge_gap, process_community_feedback

            feedback_result = process_community_feedback(
                question=question,
                rating=rating,
                correction=correction,
                sources=_sources if "_sources" in dir() else None,
            )
...
def health_check():
    ...
    status = {"status": "healthy", "timestamp": time.strftime("%Y-%m-%d %H:%M:%S UTC", time.gmtime()), "services": {}}

    # Check Pinecone
    try:
        stats = index.describe_index_stats()
        status["services"]["pinecone"] = {"status": "connected", "vectors": stats.get("total_vector_count", 0)}
    except Exception as e:
    ...
    try:
        ...
        status["services"]["database"] = {"status": "connected", "backend": "postgresql" if is_postgres() else "sqlite"}
    except Exception as e:
