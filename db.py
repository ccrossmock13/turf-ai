...
def _get_pg_pool():
    ...
    if _pg_pool is None and _DATABASE_URL:
        try:
            ...
            _pg_pool = pool.ThreadedConnectionPool(minconn=2, maxconn=20, dsn=_DATABASE_URL)
...
def _convert_sqlite_to_pg(sql):
    ...
    sql = re.sub(r"GROUP_CONCAT\((\w+)\)", r"STRING_AGG(CAST(\1 AS TEXT), ',')", sql, flags=re.IGNORECASE)
...
def table_exists(conn, table_name):
    ...
    if is_postgres():
        cursor = conn.execute(
            "SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = %s)", (table_name,)
        )
        row = cursor.fetchone()
