...
def generate_single_spray_record(application, course_name=""):
    ...
    doc = SimpleDocTemplate(buffer, pagesize=letter, topMargin=50, bottomMargin=40, leftMargin=36, rightMargin=36)
...
def generate_seasonal_report(applications, nutrient_summary, course_name="", date_range=""):
    ...
    if applications:
        ...
        for a in applications:
            ...
            app_rows.append(
                [a.get("date", ""), a.get("area", "").capitalize(), product_str, rate_str, total_str, cat_str]
            )
    ...
    for area_name in ["greens", "fairways", "tees", "rough"]:
        ...
        for key in NUTRIENT_KEYS:
            ...
            if val > 0 or total_val > 0:
                nut_rows.append([NUTRIENT_LABELS.get(key, key), f"{val:.3f} lbs", f"{total_val:.2f} lbs"])
        ...
        if isinstance(pj, list) and len(pj) > 1:
            ...
            for p in pj:
                ...
                if pname not in product_usage:
                    product_usage[pname] = {"category": p.get("product_category", ""), "count": 0, "areas": set()}
                product_usage[pname]["count"] += 1
                product_usage[pname]["areas"].add(area)
        else:
            pname = a.get("product_name", "Unknown")
            if pname not in product_usage:
                product_usage[pname] = {"category": a.get("product_category", ""), "count": 0, "areas": set()}
            product_usage[pname]["count"] += 1
            product_usage[pname]["areas"].add(area)
...
def generate_nutrient_report(nutrient_summary, course_name="", year=""):
    ...
    for area_name in ["greens", "fairways", "tees", "rough"]:
        ...
        elements.append(Paragraph(f'{app_count} application{"s" if app_count != 1 else ""}', styles["SmallText"]))
        elements.append(Spacer(1, 6))
        ...
        budget_data = [
            ["N Applied:", f"{n_applied:.2f} lbs/1000 sq ft", "Target:", f"{n_target} lbs/1000 sq ft"],
            ["Remaining:", f"{n_remaining:.2f} lbs/1000 sq ft", "Progress:", f"{n_pct:.0f}%"],
        ]
    ...
    if not any(areas_data.get(a, {}).get("applications_count", 0) > 0 for a in ["greens", "fairways", "tees", "rough"]):
