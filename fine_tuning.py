...
def start_fine_tuning_job(training_file_id: str, suffix: str = None, validation_file_id: str = None) -> Optional[Dict]:
    ...
    try:
        with get_db(FEEDBACK_DB) as conn:
            ...
            for row in cursor.fetchall():
                results[row[0]] = {"score": row[1], "positive": row[2], "negative": row[3]}
...
def get_low_quality_sources(threshold: float = 0.4) -> List[Dict]:
    ...
    try:
        with get_db(FEEDBACK_DB) as conn:
            ...
            return [
                {"name": row[0], "url": row[1], "score": row[2], "positive": row[3], "negative": row[4]}
                for row in cursor.fetchall()
            ]
...
def run_evaluation(custom_questions: List[Dict] = None, use_internal: bool = True) -> Dict:
    ...
    if use_internal:
        ...
        for q in questions:
            try:
                response = client.post("/ask", json={"question": q["question"]}, content_type="application/json")

                if response.status_code != 200:
                    results.append(
                        {"question": q["question"], "success": False, "error": f"API error: {response.status_code}"}
                    )
                    continue
                ...
                results.append({"question": q["question"], "success": False, "error": str(e)})
    else:
        ...
        for q in questions:
            try:
                # Call the API
                response = requests.post("http://localhost:5000/ask", json={"question": q["question"]}, timeout=60)
    ...
    summary = {
        ...
        "avg_keyword_score": round(sum(r["keyword_score"] for r in successful) / len(successful), 1)
        if successful
        else 0,
        "avg_product_score": round(sum(r["product_score"] for r in successful) / len(successful), 1)
        if successful
        else 0,
        "avg_overall_score": round(sum(r["overall_score"] for r in successful) / len(successful), 1)
        if successful
        else 0,
