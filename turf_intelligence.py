...
SEASONAL_CONTEXT = {
    1: {
        "season": "winter",
        "focus": "dormancy management, snow mold prevention, equipment maintenance",
        "active_diseases": ["snow mold", "pink snow mold"],
        "key_tasks": "Review spray programs, calibrate equipment, plan spring applications",
    },
    2: {
        "season": "late_winter",
        "focus": "pre-season planning, pre-emergent timing approaching",
        "active_diseases": ["snow mold"],
        "key_tasks": "Order chemicals, soil testing, pre-emergent timing based on forsythia bloom",
    },
    3: {
        "season": "early_spring",
        "focus": "pre-emergent application window, green-up, initial mowing",
        "active_diseases": ["snow mold recovery", "spring dead spot"],
        "key_tasks": "Apply pre-emergents at soil temp 55°F, begin mowing, first fertilizer",
    },
    4: {
        "season": "spring",
        "focus": "active growth begins, pre-emergent window closing, disease pressure rising",
        "active_diseases": ["dollar spot", "red thread", "leaf spot"],
        "key_tasks": "Topdressing, aerification, PGR programs begin, fungicide preventatives",
    },
    5: {
        "season": "late_spring",
        "focus": "peak growth, disease pressure increasing, post-emergent herbicides",
        "active_diseases": ["dollar spot", "brown patch", "anthracnose"],
        "key_tasks": "Fungicide rotations, mowing height management, irrigation scheduling",
    },
    6: {
        "season": "early_summer",
        "focus": "heat stress approaching, disease pressure high, insect scouting",
        "active_diseases": ["dollar spot", "brown patch", "pythium", "summer patch", "anthracnose"],
        "key_tasks": "Syringing, raise mowing heights, curative sprays if needed",
    },
    7: {
        "season": "summer",
        "focus": "peak heat stress, maximum disease pressure, water management critical",
        "active_diseases": ["pythium", "brown patch", "summer patch", "anthracnose", "fairy ring"],
        "key_tasks": "Hand watering, fans on greens, avoid traffic stress, monitor closely",
    },
    8: {
        "season": "late_summer",
        "focus": "continued heat stress, fall prep begins, overseeding planning",
        "active_diseases": ["pythium", "brown patch", "gray leaf spot", "summer patch"],
        "key_tasks": "Plan fall aerification, seed ordering, reduce N on bentgrass",
    },
    9: {
        "season": "early_fall",
        "focus": "overseeding window, aerification, fall fertilization",
        "active_diseases": ["dollar spot", "brown patch", "gray leaf spot"],
        "key_tasks": "Core aerify, overseed, fall N program, pre-emergent for Poa annua",
    },
    10: {
        "season": "fall",
        "focus": "fall recovery, winterization prep, final fungicide applications",
        "active_diseases": ["dollar spot", "leaf spot"],
        "key_tasks": "Winterizer fertilizer, snow mold prevention spray, lower HOC gradually",
    },
    11: {
        "season": "late_fall",
        "focus": "winterization, snow mold prevention, final mowing",
        "active_diseases": ["snow mold risk building"],
        "key_tasks": "Snow mold preventative sprays, drain irrigation, store equipment",
    },
    12: {
        "season": "winter",
        "focus": "dormancy, equipment overhaul, planning for next season",
        "active_diseases": ["snow mold under cover"],
        "key_tasks": "Equipment maintenance, budget planning, education/conferences",
    },
}
...
def build_seasonal_context(month=None, day=None, state=None, grass_type=None):
    ...
    zone = get_turfgrass_zone(state) or (
        "warm_season"
        if grass_type
        and any(
            g in (grass_type or "").lower()
            for g in ["bermuda", "zoysia", "paspalum", "st augustine", "centipede", "bahia"]
        )
        else "cool_season"
    )
    ...
    if zone == "transition":
        parts.append(
            "Note: Transition zone — both cool-season and warm-season turf may be present. Tailor advice to the specific grass type."
        )
...
FRAC_CODE_MAP = {
    ...
    "FRAC3": {
        "group": "DMI (Triazoles)",
        "products": ["banner maxx", "bayleton", "tourney", "maxtima", "propiconazole", "myclobutanil"],
        "resistance_risk": "medium",
    },
    "FRAC7": {
        "group": "SDHI (Carboxamides)",
        "products": ["xzemplar", "velista", "posterity", "lexicon", "briskway"],
        "resistance_risk": "medium-high",
    },
    "FRAC11": {
        "group": "QoI (Strobilurins)",
        "products": ["heritage", "insignia", "headway", "compass", "disarm"],
        ...
        if proposed_product:
            ...
            if proposed_frac and recent_frac:
                ...
                if consecutive_same >= 2:
                    ...
                    if alternatives:
                        result["suggestion"] = (
                            f"Consider rotating to a different mode of action: {', '.join(alt.title() for alt in alternatives[:4])}"
                        )
...
def build_frac_rotation_context(user_id, question, area=None):
    ...
    parts.append("Recent fungicide applications (last 60 days):")
...
def assess_spray_window(weather_data):
    ...
    if temp > 90:
        ...
        recommendations.append(
            "Avoid DMI fungicides and chlorothalonil above 85°F. Consider early morning application (before 9 AM)."
        )
    elif temp > 85:
        phyto_risk = True
...
def build_cross_module_context(user_id, question, area=None):
    ...
    try:
        ...
        with get_db() as conn:
            ...
            if any(
                kw in q_lower
                for kw in ["diagnose", "issue", "problem", "disease", "pest", "weed", "damage", "scout", "identify"]
            ):
                try:
            ...
            if any(
                kw in q_lower
                for kw in ["nitrogen", "fertiliz", "nutrient", "n budget", "how much n", "fertilizer program"]
            ):
                try:
...
TANK_MIX_COMPATIBILITY = {
    ...
    ("banner maxx", "daconil"): {
        "status": "caution",
        "notes": "Avoid above 85°F — phytotoxicity risk with DMI + contact in heat",
    },
    ...
    ("prodiamine", "dimension"): {
        "status": "incompatible",
        "notes": "Do not combine two pre-emergents — redundant and may cause phytotoxicity",
    },
    ("specticle", "barricade"): {"status": "incompatible", "notes": "Do not combine two pre-emergents"},
    # High-heat warnings
    ("banner maxx", "heritage"): {
        "status": "caution",
        "notes": "DMI + strobilurin may increase phytotoxicity risk above 85°F",
    },
                ...
                alerts.append(
                    {
                        ...
                        "severity": "warning",
                        "message": "Brown patch conditions: night temps >68°F with high humidity. Avoid late-day nitrogen and excessive irrigation.",
                    }
                )
