...
class TestIrrigationZoneCRUD:
    ...
    def test_create_zone_auto_precipitation_rate(self, auth_client):
        ...
        resp = self._create_zone(auth_client, name="Auto Calc Zone", heads_count=10, gpm_per_head=12.0, area_sqft=6000)
        assert resp.status_code == 201
        data = resp.get_json()
    ...
    def test_update_zone(self, auth_client):
        ...
        resp = auth_client.put(
            f"/api/irrigation/zones/{zone_id}",
            json={
                "name": "Updated Name",
                "soil_type": "clay_loam",
                "notes": "Updated notes",
