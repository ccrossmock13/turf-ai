...
class TestAskEndpointBasics:
    ...
    def test_empty_question_returns_prompt(self, app_client):
        ...
        response = app_client.post("/ask", data=json.dumps({"question": ""}), content_type="application/json")
...
class TestAskEndpointPipeline:
    ...
    def test_full_pipeline_returns_answer(
        self,
        mock_grounding,
        mock_feasibility,
        mock_rewrite,
        ...
        with patch("app.openai_client") as mock_openai_client:
            ...
            response = app_client.post(
                "/ask",
                data=json.dumps({"question": "What is the Heritage rate for bentgrass greens?"}),
                content_type="application/json",
            )
...
class TestAskResponseStructure:
    ...
    def test_error_response_has_required_fields(self, app_client):
        ...
        with patch("pipeline.RateLimiter") as mock_rl:
            ...
            response = app_client.post("/ask", data=json.dumps({"question": "test"}), content_type="application/json")
