...
class TestFollowUpSuggestions:
    ...
    def test_no_duplicate_of_original(self):
        ...
        suggestions = generate_follow_up_suggestions(
            "What is the rate for Heritage?", "Heritage rate is...", intent={"type": "rate"}, product="heritage"
        )
...
class TestSprayWindow:
    def test_good_conditions(self):
        ...
        result = assess_spray_window({"temp": 75, "wind_speed": 5, "humidity": 50, "rain_chance": 10})
    ...
    def test_high_wind_is_poor(self):
        ...
        result = assess_spray_window({"temp": 75, "wind_speed": 18, "humidity": 50, "rain_chance": 10})
    ...
    def test_high_rain_chance_is_poor(self):
        ...
        result = assess_spray_window({"temp": 75, "wind_speed": 5, "humidity": 50, "rain_chance": 70})
        assert result["quality"] == "poor"

    def test_high_temp_phytotoxicity(self):
        from turf_intelligence import assess_spray_window

        result = assess_spray_window({"temp": 95, "wind_speed": 5, "humidity": 50, "rain_chance": 10})
    ...
    def test_marginal_wind(self):
        ...
        result = assess_spray_window({"temp": 75, "wind_speed": 12, "humidity": 50, "rain_chance": 10})
        assert result["quality"] == "marginal"
    ...
    def test_recommendations_provided(self):
        ...
        result = assess_spray_window({"temp": 92, "wind_speed": 14, "humidity": 50, "rain_chance": 40})
        assert len(result["recommendations"]) > 0

    def test_build_spray_context(self):
        from turf_intelligence import build_weather_spray_context

        ctx = build_weather_spray_context({"temp": 75, "wind_speed": 5, "humidity": 50, "rain_chance": 10})
...
class TestPredictiveAlerts:
    def test_basic_alerts(self):
        ...
        alerts = generate_predictive_alerts(user_id=1, state="ohio", grass_type="bentgrass")
...
class TestScoringServiceUpdated:
    ...
    def test_diversity_penalty_basic(self):
        ...
        results = [
            {"text": f"text{i}", "source": "same-source.pdf", "score": 10.0 - i, "metadata": {}} for i in range(10)
        ]
    ...
    def test_diversity_preserves_different_sources(self):
        ...
        results = [
            {"text": f"text{i}", "source": f"source-{i}.pdf", "score": 10.0 - i, "metadata": {}} for i in range(5)
        ]
        penalized = _apply_diversity_penalty(results)
...
class TestIntegration:
    ...
    def test_disease_pressure_and_season(self):
        ...
        from turf_intelligence import SEASONAL_CONTEXT, get_regional_disease_pressure
    ...
    def test_follow_ups_from_intent(self):
        ...
        disease_followups = generate_follow_up_suggestions(
            "Dollar spot", "Use Heritage...", intent={"type": "disease"}, disease="dollar spot"
        )
        rate_followups = generate_follow_up_suggestions(
            "Heritage rate", "0.4 oz...", intent={"type": "rate"}, product="heritage"
        )
