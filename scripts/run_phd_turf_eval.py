"""Run a curated expert-level turf evaluation set against /ask."""
...
os.environ.setdefault("DATA_DIR", tempfile.mkdtemp(prefix="turf-ai-phd-eval-"))

from app import RATE_LIMIT_BUCKETS, app  # noqa: E402
from auth_store import create_account, mark_email_verified  # noqa: E402


DEFAULT_CASES_PATH = ROOT / "scripts" / "phd_turf_eval_cases.json"
DEFAULT_PROFILE = {
...
def _make_client():
    ...
    with client.session_transaction() as session:
        session["_csrf_token"] = "phd-turf-eval-csrf"

    email = f"phd-eval-{uuid.uuid4().hex[:8]}@example.com"
    ...
    login_response = client.post(
        "/login",
        json={"email": email, "password": password, "csrf_token": "phd-turf-eval-csrf"},
    )
    if login_response.status_code != 200:
        raise RuntimeError(f"Failed to log in eval account: HTTP {login_response.status_code}")
    with client.session_transaction() as session:
        token = session.get("_csrf_token") or "phd-turf-eval-csrf"
...
def main() -> int:
    ...
    parser.add_argument("--cases", default=str(DEFAULT_CASES_PATH), help="Path to the PhD turf eval case file.")
