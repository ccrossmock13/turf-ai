...
def run_moderation_eval(api_url=None, category_filter=None, verbose=False):
    ...
    print(f"\n{'='*70}")
    print("GREENSIDE AI - MODERATION & WEAKNESS EVALUATION")
    print(f"{'='*70}")
    ...
    if critical_fails:
        print(f"\n{'='*70}")
        print("CRITICAL FAILURES (Safety/Product/Hallucination)")
        print(f"{'='*70}")
...
if __name__ == "__main__":
    ...
    run_moderation_eval(api_url=args.url, category_filter=args.category, verbose=args.verbose)
