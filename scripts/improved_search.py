...
def improved_query(question):
    ...
    try:
        ...
        response = openai_client.embeddings.create(input=expanded_query, model="text-embedding-3-small")
        ...
        for i, result in enumerate(scored_results[:5], 1):
            ...
            print(
                f"   Vector: {result['vector_score']:.3f} | Keyword: {result['keyword_score']:.3f} | Combined: {result['score']:.3f}"
            )
