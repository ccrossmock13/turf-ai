"""Run a curated live evaluation set for ambiguous turf questions against /ask."""
...
os.environ.setdefault("DATA_DIR", tempfile.mkdtemp(prefix="turf-ai-ambiguity-eval-"))

from app import RATE_LIMIT_BUCKETS, app  # noqa: E402
from auth_store import create_account  # noqa: E402


DEFAULT_CASES_PATH = ROOT / "scripts" / "ambiguity_eval_cases.json"
DEFAULT_PROFILE = {
...
def _make_client():
    ...
    with client.session_transaction() as session:
        session["_csrf_token"] = "ambiguity-eval-csrf"

    email = f"ambiguity-eval-{uuid.uuid4().hex[:8]}@example.com"
    ...
    login_response = client.post(
        "/login",
        json={"email": email, "password": password, "csrf_token": "ambiguity-eval-csrf"},
    )
    if login_response.status_code != 200:
        raise RuntimeError(f"Failed to log in eval account: HTTP {login_response.status_code}")
    with client.session_transaction() as session:
        token = session.get("_csrf_token") or "ambiguity-eval-csrf"
...
def main() -> int:
    ...
    parser.add_argument("--cases", default=str(DEFAULT_CASES_PATH), help="Path to the ambiguity eval case file.")
