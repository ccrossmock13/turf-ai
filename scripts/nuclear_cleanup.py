...
for match in results["matches"]:
    ...
    is_syngenta_program = any(
        keyword in name.lower() for keyword in ["greens", "fairway", "tees", "bermudagrass", "bentgrass", "day"]
    )
    ...
    if delete_this:
        to_delete.append({"id": vector_id, "name": name, "type": doc_type, "reason": reason})
    else:
        to_keep.append({"id": vector_id, "name": name, "type": doc_type})
...
print("Restart Flask and check it out.")
