...
HEADERS = {"Content-Type": "application/json", "Accept": "application/json", "User-Agent": "GreensideAI-Eval/1.0"}
...
def run_evaluation(api_url=None):
    ...
    print(f"\n{'='*60}")
    print("GREENSIDE AI - 100 QUESTION EVALUATION")
    ...
    for i, q in enumerate(EVAL_QUESTIONS_100, 1):
        ...
        try:
            response = requests.post(api_url, json={"question": question}, headers=HEADERS, timeout=TIMEOUT)
            ...
            print("❌ TIMEOUT")
            failed += 1
            results.append({"question": question, "category": q["category"], "success": False, "error": "Timeout"})
        except Exception as e:
            print(f"❌ {str(e)[:30]}")
            failed += 1
            results.append({"question": question, "category": q["category"], "success": False, "error": str(e)})
    ...
    summary = {
        ...
        "avg_confidence": round(sum(r["confidence"] for r in successful_results) / len(successful_results), 1)
        if successful_results
        else 0,
        "avg_keyword_score": round(sum(r["keyword_score"] for r in successful_results) / len(successful_results), 1)
        if successful_results
        else 0,
        "avg_product_score": round(sum(r["product_score"] for r in successful_results) / len(successful_results), 1)
        if successful_results
        else 0,
        "avg_overall_score": round(sum(r["overall_score"] for r in successful_results) / len(successful_results), 1)
        if successful_results
        else 0,
    ...
    for cat, data in sorted(categories.items()):
        print(
            f"  {cat:15s}: {data['avg_score']:5.1f}% score, {data['avg_confidence']:5.1f}% conf ({data['count']} questions)"
        )
