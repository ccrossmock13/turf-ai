...
for url, vectors in url_to_vectors.items():
    ...
    if status == "working":
        working.append({"url": url, "vectors": vectors})
    else:
        broken.append({"url": url, "vectors": vectors, "error": error})
    ...
    for item in broken:
        for vector_info in item["vectors"]:
            try:
                ...
                index.upsert(
                    vectors=[{"id": vector_info["id"], "values": vector_data["values"], "metadata": updated_metadata}]
                )
