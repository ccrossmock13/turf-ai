...
if choice == "1":
    ...
    results = index.query(vector=[0.0] * 1536, top_k=10000, include_metadata=True)
    ...
    for match in results["matches"]:
        ...
        if any(pattern in name for pattern in ["EPA Label:", "Pesticide Label:", "research-", "chunk-", "_", "--"]):
            to_fix.append({"id": match["id"], "old_name": name, "metadata": metadata})
            ...
            for item in to_fix:
                ...
                if new_name != old_name:
                    try:
                        ...
                        index.upsert(
                            vectors=[{"id": item["id"], "values": vector_data["values"], "metadata": updated_metadata}]
                        )
    ...
    if broken:
        ...
        remove = (
            input(f"\nRemove pdf_path/label_url from {sum(len(b['vector_ids']) for b in broken)} vectors? (yes/no): ")
            .strip()
            .lower()
        )
            ...
            for item in broken:
                for vector_id in item["vector_ids"]:
                    try:
                        ...
                        index.upsert(
                            vectors=[{"id": vector_id, "values": vector_data["values"], "metadata": updated_metadata}]
                        )
    ...
    results = index.query(
        vector=[0.0] * 1536, top_k=10000, filter={"type": {"$in": types_to_hide}}, include_metadata=True
    )
        ...
        if sub_choice == "1":
            ...
            for match in results["matches"]:
                try:
                    ...
                    index.upsert(
                        vectors=[{"id": match["id"], "values": vector_data["values"], "metadata": updated_metadata}]
                    )
