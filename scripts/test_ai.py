...
def test_question(question, expected, should_not_contain=None, check_source=None):
    ...
    try:
        response = requests.post(f"{BASE_URL}/ask", json={"question": question}, timeout=30)

        if response.status_code != 200:
            return {"status": "FAIL", "error": f"HTTP {response.status_code}", "answer": None, "sources": []}
        ...
        return {"status": "ERROR", "error": str(e), "answer": None, "sources": []}
...
for category, tests in test_suite.items():
    ...
    for test in tests:
        ...
        result = test_question(question, test["expected"], test.get("should_not_contain"), test.get("check_source"))

        # Store result
        RESULTS.append({"category": category, "question": question, **result})

        # Display result
        status_symbol = {"PASS": "✅", "PARTIAL": "⚠️", "FAIL": "❌", "ERROR": "💥"}
