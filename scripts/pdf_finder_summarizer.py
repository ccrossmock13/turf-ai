        ...
        if "greencast" in url.lower():
            ...
            for link in links:
                try:
                    ...
                    if href and (
                        ".pdf" in href.lower()
                        or "label" in text.lower()
                        or "sds" in text.lower()
                        or "sheet" in text.lower()
                    ):
                        pdf_links.append(
                            {
                                "url": href,
...
Summary:"""
            ...
            if user_choice == "1":
                ...
                if suggested_name:
                    approved.append(
                        {"content": pdf_response.content, "filename": suggested_name + ".pdf", "url": pdf["url"]}
                    )
                    print(f"  ✅ Will save as: {suggested_name}.pdf")
                else:
                    print("  ⚠️  No filename entered, skipping")
            else:
                print("  ⏭️  Skipped")
