...
def calculate_gdd(base_temp, daily_highs, daily_lows):
    ...
    if len(daily_highs) != len(daily_lows):
        raise ValueError(
            "daily_highs and daily_lows must have the same length " f"({len(daily_highs)} != {len(daily_lows)})"
        )

    total = 0.0
    for high, low in zip(daily_highs, daily_lows, strict=False):
        avg = (high + low) / 2.0
...
def _cool_season(month):
    ...
    if month in (11, 12, 1, 2):
        return {
            "season": "dormant",
            "description": (
                "Turf is dormant. Avoid traffic on frozen turf. Plan for " "spring pre-emergent applications."
            ),
        }
    if month in (3, 4):
        return {
...
def _warm_season(month):
    ...
    return {
        ...
        "description": (
            "Growth is slowing. Final fertilization window. Consider " "overseeding with ryegrass for winter color."
        ),
    }
...
def _transition_zone_season(month):
    ...
    return {
        "season": "fall_transition",
        "description": (
            "Growth winding down. Apply winterizer fertilizer. Last " "mowing of the season for warm-season turf."
        ),
    }
