...
def retire_equipment(equip_id, user_id):
    ...
    with get_db() as conn:
        cursor = conn.execute(
            "UPDATE equipment SET status = 'retired', updated_at = CURRENT_TIMESTAMP " "WHERE id = ? AND user_id = ?",
            (equip_id, user_id),
        )
...
def delete_equipment(equip_id, user_id):
    ...
    with get_db() as conn:
        ...
        row = conn.execute("SELECT id FROM equipment WHERE id = ? AND user_id = ?", (equip_id, user_id)).fetchone()
        if not row:
            return False
...
def get_equipment_by_id(equip_id, user_id):
    ...
    """
    with get_db() as conn:
        cursor = conn.execute("SELECT * FROM equipment WHERE id = ? AND user_id = ?", (equip_id, user_id))
        row = cursor.fetchone()
...
def log_maintenance(user_id, data):
    ...
    with get_db() as conn:
        # Verify ownership
        cursor = conn.execute("SELECT id FROM equipment WHERE id = ? AND user_id = ?", (equip_id, user_id))
        ...
        if data.get("set_status") and data["set_status"] in VALID_STATUSES:
            conn.execute(
                "UPDATE equipment SET status = ?, updated_at = CURRENT_TIMESTAMP " "WHERE id = ? AND user_id = ?",
...
def create_maintenance_schedule(user_id, data):
    ...
    with get_db() as conn:
        # Verify ownership
        cursor = conn.execute(
            "SELECT id, current_hours FROM equipment WHERE id = ? AND user_id = ?", (equip_id, user_id)
        )
...
def _recalculate_equipment_hours(conn, equip_id):
    ...
    """
    cursor = conn.execute(
        "SELECT COALESCE(SUM(hours_used), 0) AS total FROM equipment_hours WHERE equipment_id = ?", (equip_id,)
    )
    row = cursor.fetchone()
    total = float(row["total"]) if row else 0.0

    conn.execute(
        "UPDATE equipment SET current_hours = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?", (total, equip_id)
    )
    return total
...
def get_calibrations_due(user_id):
    ...
    for row in rows:
        r = dict(row)
        r["is_overdue"] = r.get("next_calibration_date", "") < datetime.now().strftime("%Y-%m-%d")
        # Parse JSON fields
        if r.get("settings_json"):
            try:
                r["settings_json"] = json.loads(r["settings_json"])
...
def get_fleet_summary(user_id):
    ...
    with get_db() as conn:
        ...
        # Total
        cursor = conn.execute("SELECT COUNT(*) AS total FROM equipment WHERE user_id = ?", (user_id,))
        total_row = cursor.fetchone()
        total = total_row["total"] if total_row else 0

    return {"total": total, "by_type": by_type, "by_status": by_status, "by_area": by_area}
...
def get_maintenance_forecast(user_id):
    ...
    forecast = {"overdue": [], "next_7_days": [], "next_30_days": [], "next_90_days": []}
