...
VALID_CATEGORIES = ["chemical", "fertilizer", "seed", "equipment", "labor", "irrigation", "other"]
...
def create_budget(user_id, data):
    ...
    with get_db() as conn:
        cursor = conn.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
            (user_id, name, fiscal_year, total_amount, category, area, notes, now, now),
        )
    ...
    logger.info("Budget created: id=%s user=%s name=%s year=%s", budget_id, user_id, name, fiscal_year)
    return budget_id
...
def update_budget(budget_id, user_id, data):
    ...
    with get_db() as conn:
        cursor = conn.execute(f"UPDATE budgets SET {', '.join(fields)} WHERE id = ? AND user_id = ?", params)
...
def delete_budget(budget_id, user_id):
    ...
    with get_db() as conn:
        ...
        conn.execute("DELETE FROM budget_line_items WHERE budget_id = ?", (budget_id,))
        cursor = conn.execute("DELETE FROM budgets WHERE id = ? AND user_id = ?", (budget_id, user_id))
...
def get_budget_by_id(budget_id, user_id):
    ...
    """
    with get_db() as conn:
        cursor = conn.execute("SELECT * FROM budgets WHERE id = ? AND user_id = ?", (budget_id, user_id))
        row = cursor.fetchone()
        if not row:
            return None
        ...
        li_cursor = conn.execute("SELECT * FROM budget_line_items WHERE budget_id = ? ORDER BY month, id", (budget_id,))
...
def create_purchase_order(user_id, data):
    ...
    logger.info("Purchase order created: id=%s po=%s user=%s vendor=%s", po_id, po_number, user_id, vendor)
    ...
    with get_db() as conn:
        ...
        if "line_items" in data:
            ...
            if "tax" not in data or "shipping" not in data:
                cur = conn.execute(
                    "SELECT tax, shipping FROM purchase_orders WHERE id = ? AND user_id = ?", (po_id, user_id)
                )
        ...
        cursor = conn.execute(f"UPDATE purchase_orders SET {', '.join(fields)} WHERE id = ? AND user_id = ?", params)
        updated = cursor.rowcount > 0
...
def update_po_status(po_id, user_id, status):
    ...
    if updated:
        logger.info("PO status updated: id=%s status=%s user=%s", po_id, valid_status, user_id)
    return updated
...
def get_po_by_id(po_id, user_id):
    ...
    """
    with get_db() as conn:
        cursor = conn.execute("SELECT * FROM purchase_orders WHERE id = ? AND user_id = ?", (po_id, user_id))
        row = cursor.fetchone()
        if not row:
            return None
        ...
        li_cursor = conn.execute("SELECT * FROM po_line_items WHERE po_id = ? ORDER BY id", (po_id,))
...
def log_expense(user_id, data):
    ...
    with get_db() as conn:
        cursor = conn.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
            (user_id, expense_date, amount, category, description, vendor, po_id, receipt_url, area, now),
        )
        expense_id = cursor.lastrowid

    logger.info("Expense logged: id=%s user=%s amount=%.2f category=%s", expense_id, user_id, amount, category)
    return expense_id
...
def get_cost_per_acre(user_id, fiscal_year, area=None):
    ...
    for r in rows:
        ...
        areas[area_name] = {"total_spent": round(total_spent, 2), "acreage": acreage, "cost_per_acre": cost_per}
...
def get_budget_vs_actual(user_id, fiscal_year):
    ...
    for cat in sorted(all_cats):
        ...
        monthly = {}
        for m in MONTHS:
            monthly[m] = {"budgeted": round(b["monthly"].get(m, 0), 2), "actual": round(a["monthly"].get(m, 0), 2)}
...
def forecast_spending(user_id, fiscal_year):
    ...
    for r in rows:
        ...
        by_category[cat] = {"ytd": round(ytd, 2), "avg_monthly": avg_m, "projected": projected}
        ytd_total += ytd
...
def get_product_spend(user_id, fiscal_year):
    ...
    return {"fiscal_year": fiscal_year, "products": products}
...
def get_vendor_summary(user_id, fiscal_year):
    ...
    for r in po_rows:
        ...
        if v not in vendor_map:
            vendor_map[v] = {"po_total": 0, "po_count": 0, "exp_total": 0, "exp_count": 0}
    ...
    return {"fiscal_year": fiscal_year, "vendors": vendors}
...
def update_expense(user_id, expense_id, data):
    ...
    with get_db() as conn:
        ...
        fields = []
        params = []
        for key in ("date", "amount", "category", "description", "vendor", "po_id", "receipt_url", "area"):
            if key in data:
                val = data[key]
                if key == "amount":
                    val = float(val)
        ...
        conn.execute(f'UPDATE expenses SET {", ".join(fields)} WHERE id = ? AND user_id = ?', params)
...
def delete_purchase_order(user_id, po_id):
    ...
    """
    with get_db() as conn:
        cursor = conn.execute("DELETE FROM purchase_orders WHERE id = ? AND user_id = ?", (po_id, user_id))
    deleted = cursor.rowcount > 0
    if deleted:
...
def get_line_items(budget_id, user_id):
    ...
    with get_db() as conn:
        ...
        rows = conn.execute(
            "SELECT * FROM budget_line_items WHERE budget_id = ? ORDER BY category, description", (budget_id,)
        ).fetchall()
    return [dict(r) for r in rows]
...
def delete_line_item(budget_id, item_id, user_id):
    ...
    with get_db() as conn:
        ...
        if not budget:
            raise ValueError(f"Budget {budget_id} not found")
        conn.execute("DELETE FROM budget_line_items WHERE id = ? AND budget_id = ?", (item_id, budget_id))
    return {"deleted": True}
