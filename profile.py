...
def save_profile(user_id, profile_data):
    ...
    with get_db() as conn:
        ...
        cultivar_data = {}
        for key in [
            "primary_grass_cultivar",
            "greens_grass_cultivar",
            "fairways_grass_cultivar",
            "rough_grass_cultivar",
            "tees_grass_cultivar",
        ]:
...
def _unpack_profile(row):
    ...
    for json_field in [
        ...
        "secondary_grasses",
        "irrigation_schedule",
        "aerification_program",
        "topdressing_program",
        "pgr_program",
        "wetting_agent_program",
...
def get_profile(user_id, course_name=None):
    ...
    with get_db() as conn:
        ...
        if course_name:
            cursor.execute(
                "SELECT * FROM course_profiles WHERE user_id = ? AND course_name = ?", (user_id, course_name)
            )
        else:
            cursor.execute(
                "SELECT * FROM course_profiles WHERE user_id = ? ORDER BY is_active DESC, updated_at DESC LIMIT 1",
                (user_id,),
            )
        row = cursor.fetchone()
...
def get_profiles(user_id):
    ...
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT * FROM course_profiles WHERE user_id = ? ORDER BY is_active DESC, course_name", (user_id,)
        )
...
def set_active_profile(user_id, course_name):
    ...
    with get_db() as conn:
        ...
        cursor.execute(
            "UPDATE course_profiles SET is_active = 1 WHERE user_id = ? AND course_name = ?", (user_id, course_name)
        )
...
def delete_profile(user_id, course_name):
    ...
    with get_db() as conn:
        ...
        if count <= 1:
            return False
        cursor.execute("DELETE FROM course_profiles WHERE user_id = ? AND course_name = ?", (user_id, course_name))
...
def get_profile_templates():
    ...
    return [{"id": k, "label": v["label"], "description": v["description"]} for k, v in PROFILE_TEMPLATES.items()]
...
def build_profile_context(user_id, question_topic=None):
    ...
    if profile.get("cultivars"):
        try:
            cultivar_data = (
                json.loads(profile["cultivars"])
                if isinstance(profile["cultivars"], str)
                else profile.get("cultivars", {})
            )
        except (json.JSONDecodeError, TypeError):
            pass
    ...
    if _include("acreage"):
        acreage_bits = []
        for key, label in [
            ("greens_acreage", "Greens"),
            ("fairways_acreage", "Fairways"),
            ("tees_acreage", "Tees"),
            ("rough_acreage", "Rough"),
        ]:
            val = profile.get(key)
    ...
    try:
        ...
        if state_name:
            ...
            if season_info:
                parts.append(
                    f"SEASON: {season_info['season'].replace('_', ' ').title()} — {season_info['description']}"
                )
    except ImportError:
        pass
...
def save_sprayer(user_id, data):
    ...
    with get_db() as conn:
        ...
        if data.get("is_default"):
            cursor.execute("UPDATE sprayers SET is_default=0 WHERE user_id=? AND id!=?", (user_id, sprayer_id))
