...
def delete_zone(zone_id, user_id):
    ...
    """
    with get_db() as conn:
        cursor = conn.execute("DELETE FROM irrigation_zones WHERE id = ? AND user_id = ?", (zone_id, user_id))
...
def get_zone_by_id(zone_id, user_id):
    ...
    """
    with get_db() as conn:
        cursor = conn.execute("SELECT * FROM irrigation_zones WHERE id = ? AND user_id = ?", (zone_id, user_id))
        row = cursor.fetchone()
...
def get_irrigation_recommendation(user_id, zone_id=None):
    ...
    # Get latest ET data
    with get_db() as conn:
        cursor = conn.execute("SELECT * FROM et_data WHERE user_id = ? ORDER BY date DESC LIMIT 1", (user_id,))
...
def log_irrigation_run(user_id, data):
    ...
    if inches_applied is None and data.get("duration_minutes"):
        ...
        if zone and zone.get("precipitation_rate"):
            inches_applied = round((data["duration_minutes"] / 60.0) * zone["precipitation_rate"], 3)
        ...
        if values:
            ...
            if mid > 0:
                ...
                trend_direction = (
                    ...
                    else ("decreasing" if second_half_avg < first_half_avg - 1 else "stable")
                )
            else:
...
def log_et_data(user_id, data):
    ...
    if et0 is None and data.get("high_temp") is not None and data.get("low_temp") is not None:
        day_of_year = (
            datetime.strptime(data.get("date", datetime.now().strftime("%Y-%m-%d")), "%Y-%m-%d").timetuple().tm_yday
        )
...
def get_water_balance(user_id, zone_id, days=14):
    ...
    with get_db() as conn:
        cursor = conn.execute(
            "SELECT date, et0, rainfall FROM et_data " "WHERE user_id = ? AND date >= ? ORDER BY date ASC",
            (user_id, cutoff),
        )
...
def get_water_cost_summary(user_id, year=None):
    ...
    return {
        ...
        "cost_per_1000_gallons": round((total_cost / total_gallons) * 1000, 2) if total_gallons > 0 else 0,
    }
...
def delete_water_usage(usage_id, user_id):
    ...
    with get_db() as conn:
        conn.execute("DELETE FROM water_usage WHERE id = ? AND user_id = ?", (usage_id, user_id))
    return {"deleted": True}
