...
def generate_monthly_report(user_id, year, month):
    ...
    for app in applications:
        entry = {
            ...
            "product_category": app.get("product_category"),
            "rate": app["rate"],
            "rate_unit": app["rate_unit"],
            "area_acreage": app.get("area_acreage"),
            "total_product": app.get("total_product"),
            "total_product_unit": app.get("total_product_unit"),
            "weather_temp": app.get("weather_temp"),
            "weather_wind": app.get("weather_wind"),
            "notes": app.get("notes"),
        }
        if app.get("products_json") and isinstance(app["products_json"], list):
    ...
    content = {
        "report_type": "monthly_super",
        "year": year,
        "month": month,
        "month_name": month_name,
        "date_range": {"start": start_date, "end": end_date},
        "weather_summary": weather_summary,
        "spray_applications": spray_summary,
        "spray_count": len(spray_summary),
        "products_by_category": products_by_category,
        "nutrient_summary": nutrient_summary,
        "scouting": scouting_summary,
        "budget": budget_summary,
    }
    report_id = _save_report_record(user_id, "monthly_super", title, start_date, end_date, content)
    content["report_id"] = report_id
    logger.info(
        f"Monthly report generated: {title} (user={user_id}, sprays={len(spray_summary)}, scouting={len(scouting)})"
    )
    return content
...
def generate_green_committee_report(user_id, year, month):
    ...
    for r in resolved_issues:
        accomplishments.append(
            f"Resolved {r.get('issue_type', 'issue')} on {r.get('area', 'course')} ({r.get('scout_date', 'N/A')})"
        )
    categories_used = {}
    for app in applications:
        cat = app.get("product_category", "other")
    ...
    weather_impact = {
        ...
        "avg_temp": round(sum(temps) / len(temps), 1) if temps else None,
        "high_wind_days": len(
            [a for a in applications if a.get("weather_wind") and _parse_wind_speed(a["weather_wind"]) > 10]
        ),
    }
        ...
        issues_and_resolutions.append(
            {
                ...
                "severity": r.get("severity"),
                "status": r.get("status"),
                "treatment": r.get("treatment_applied"),
            }
        )

    content = {
        "report_type": "green_committee",
        "year": year,
        "month": month,
        "month_name": month_name,
        "date_range": {"start": start_date, "end": end_date},
        "conditions_overview": conditions_overview,
        "accomplishments": accomplishments,
        "upcoming_plans": upcoming_plans,
        "budget_summary": {
            "total_budgeted": budget_summary["total_budgeted"],
            "total_spent": budget_summary["total_spent"],
            "pct_used": budget_summary["pct_used"],
        },
        "weather_impact": weather_impact,
        "issues_and_resolutions": issues_and_resolutions,
    }
...
def generate_annual_report(user_id, year):
        ...
        monthly_summaries.append(
            {
                ...
                "applications_count": len(m_apps),
                "scouting_reports_count": len(m_scout),
    ...
    total_chemicals = {
        "application_count": len(chemical_apps),
        "products_used": sorted(set(a["product_name"] for a in chemical_apps)),
    }
    total_fertilizer = {
        "application_count": len(fertilizer_apps),
        "products_used": sorted(set(a["product_name"] for a in fertilizer_apps)),
        "nutrients": _aggregate_nutrients(fertilizer_apps),
    }
    ...
    yoy_comparison = {
        ...
        "previous_applications": len(prev_applications),
        "current_scouting": len(scouting),
        "previous_scouting": len(prev_scouting),
        "current_spend": budget_perf["total_spent"],
        "previous_spend": prev_total_spent,
        "spend_change_pct": round(((budget_perf["total_spent"] - prev_total_spent) / prev_total_spent * 100), 1)
    ...
    content = {
        ...
        "nutrient_totals": _aggregate_nutrients(applications),
        "budget_performance": budget_perf,
        "yoy_comparison": yoy_comparison,
        "total_applications": len(applications),
        "total_scouting_reports": len(scouting),
    }
    report_id = _save_report_record(user_id, "annual", title, start_date, end_date, content)
    content["report_id"] = report_id
    logger.info(
        f"Annual report generated: {title} (user={user_id}, apps={len(applications)}, expenses={len(expenses)})"
    )
    return content
...
def generate_pesticide_compliance_report(user_id, year):
    ...
    for app in applications:
        ...
        record = {
            ...
            "weather_temp": app.get("weather_temp"),
            "weather_wind": app.get("weather_wind"),
            "weather_conditions": app.get("weather_conditions"),
            "notes": app.get("notes"),
        }
        if app.get("products_json") and isinstance(app["products_json"], list):
            record["tank_mix_products"] = [
                {"product_name": p.get("product_name"), "rate": p.get("rate"), "rate_unit": p.get("rate_unit")}
        ...
        compliance_details.append(
            {
                ...
                "epa_reg_number": cr.get("epa_reg_number"),
                "applicator_name": cr.get("applicator_name"),
                "applicator_license": cr.get("applicator_license"),
                "target_pest": cr.get("target_pest"),
                "area_treated": cr.get("area_treated"),
                "area_sqft": cr.get("area_sqft"),
                "rate_applied": cr.get("rate_applied"),
                "rate_unit": cr.get("rate_unit"),
                "wind_speed": cr.get("wind_speed"),
                "temperature": cr.get("temperature"),
                "rei_hours": cr.get("rei_hours"),
    ...
    for cr in compliance_rows:
        ...
        if rei is not None and rei <= 0:
            rei_issues.append(
                {
                    "date": cr["date"],
                    "product": cr.get("product_name"),
                    "rei_hours": rei,
                    "issue": "REI hours recorded as zero or negative",
                }
            )
    ...
    for cr in compliance_rows:
        ...
        if name:
            if name not in applicators:
                applicators[name] = {
                    "license": cr.get("applicator_license"),
                    "application_count": 0,
                    "products_applied": set(),
                }
    ...
    content = {
        "report_type": "pesticide_compliance",
        "year": year,
        "date_range": {"start": start_date, "end": end_date},
        ...
        "rei_issues": rei_issues,
        "applicators": applicators,
    }
    report_id = _save_report_record(user_id, "pesticide_compliance", title, start_date, end_date, content)
    content["report_id"] = report_id
    logger.info(
        f"Pesticide compliance report generated: {title} (user={user_id}, apps={len(application_records)}, compliance={len(compliance_details)})"
    )
    return content
    ...
    with get_db() as conn:
        ...
        """,
            (
                ...
                data.get("applicator_name"),
                data.get("applicator_license"),
                data.get("product_name"),
                data.get("epa_reg_number"),
                data.get("target_pest"),
                data.get("area_treated"),
                data.get("area_sqft"),
                data.get("rate_applied"),
                data.get("rate_unit"),
                data.get("wind_speed"),
...
def check_compliance_gaps(user_id, year):
    ...
    with get_db() as conn:
        applications = _fetch_spray_applications(conn, user_id, start_date, end_date)
        cursor = conn.execute(
            "SELECT * FROM compliance_records WHERE user_id = ? AND date >= ? AND date <= ?",
            (user_id, start_date, end_date),
        )
            ...
            missing_compliance.append(
                {
                    ...
                    "product_name": app["product_name"],
                    "area": app["area"],
                    "issue": "No compliance record found for this application",
                }
            )
    ...
    return {
        ...
        "missing_compliance_count": len(missing_compliance),
...
def _render_monthly_html(content):
    ...
    if sprays:
        ...
        for s in sprays:
            ...
            parts.append(
                f'<tr><td>{_html_escape(s.get("date", ""))}</td><td>{_html_escape(s.get("area", ""))}</td><td>{_html_escape(pname)}</td><td>{s.get("rate", "")} {_html_escape(s.get("rate_unit", ""))}</td><td>{_html_escape(s.get("notes", "") or "")}</td></tr>'
            )
        parts.append("</table>")
    else:
    ...
    if nutrients:
        parts.append(
            "<h2>Nutrient Summary (lbs applied)</h2><table><tr><th>Area</th><th>N</th><th>P2O5</th><th>K2O</th></tr>"
        )
        for area, vals in nutrients.items():
            parts.append(
                f'<tr><td>{_html_escape(area.title())}</td><td>{vals.get("N", 0)}</td><td>{vals.get("P2O5", 0)}</td><td>{vals.get("K2O", 0)}</td></tr>'
            )
        parts.append("</table>")
    ...
    if budget:
        ...
        if by_cat:
            parts.append(
                "<table><tr><th>Category</th><th>Budgeted</th><th>Spent</th><th>Remaining</th><th>% Used</th></tr>"
            )
            for c in by_cat:
                parts.append(
                    f'<tr><td>{_html_escape(c["category"].title())}</td><td>${c["budgeted"]:,.2f}</td><td>${c["spent"]:,.2f}</td><td>${c["remaining"]:,.2f}</td><td>{c["pct_used"]}%</td></tr>'
                )
            parts.append("</table>")
    return parts
...
def _render_green_committee_html(content):
    ...
    if issues:
        parts.append(
            "<h2>Issues and Resolutions</h2><table><tr><th>Date</th><th>Area</th><th>Issue</th><th>Severity</th><th>Status</th><th>Treatment</th></tr>"
        )
        for i in issues:
            parts.append(
                f'<tr><td>{_html_escape(i.get("date", ""))}</td><td>{_html_escape(i.get("area", ""))}</td><td>{_html_escape(i.get("issue_type", ""))}</td><td>{i.get("severity", "")}</td><td>{_html_escape(i.get("status", ""))}</td><td>{_html_escape(i.get("treatment", "") or "")}</td></tr>'
            )
        parts.append("</table>")
    return parts
...
def _render_annual_html(content):
    ...
    if monthly:
        parts.append(
            "<h2>Monthly Summaries</h2><table><tr><th>Month</th><th>Applications</th><th>Scouting</th><th>Expenses</th></tr>"
        )
        for m in monthly:
            parts.append(
                f'<tr><td>{_html_escape(m.get("month_name", ""))}</td><td>{m.get("applications_count", 0)}</td><td>{m.get("scouting_reports_count", 0)}</td><td>${m.get("total_expenses", 0):,.2f}</td></tr>'
            )
        parts.append("</table>")
    ...
    parts.append("</div>")
    for label, prods in [
        ("Pesticide Products Used", chemicals.get("products_used", [])),
        ("Fertilizer Products Used", fertilizer.get("products_used", [])),
    ]:
        if prods:
    ...
    if nutrients:
        parts.append(
            "<h3>Total Nutrients Applied (lbs)</h3><table><tr><th>Area</th><th>N</th><th>P2O5</th><th>K2O</th></tr>"
        )
        for area, vals in nutrients.items():
            parts.append(
                f'<tr><td>{_html_escape(area.title())}</td><td>{vals.get("N", 0)}</td><td>{vals.get("P2O5", 0)}</td><td>{vals.get("K2O", 0)}</td></tr>'
...
def _render_compliance_html(content):
    ...
    if applicators:
        parts.append(
            "<h2>Applicator Summary</h2><table><tr><th>Applicator</th><th>License</th><th>Applications</th><th>Products</th></tr>"
        )
        for name, info in applicators.items():
            parts.append(
                f'<tr><td>{_html_escape(name)}</td><td>{_html_escape(info.get("license", ""))}</td><td>{info.get("application_count", 0)}</td><td>{_html_escape(", ".join(info.get("products_applied", [])))}</td></tr>'
            )
        parts.append("</table>")
    records = content.get("application_records", [])
    if records:
        parts.append(
            "<h2>Application Records</h2><table><tr><th>Date</th><th>Product</th><th>Rate</th><th>Area</th><th>Temp</th><th>Wind</th></tr>"
        )
        for r in records:
            parts.append(
                f'<tr><td>{_html_escape(r.get("date", ""))}</td><td>{_html_escape(r.get("product_name", ""))}</td><td>{r.get("rate", "")} {_html_escape(r.get("rate_unit", ""))}</td><td>{_html_escape(r.get("area", ""))}</td><td>{r.get("weather_temp", "")}</td><td>{_html_escape(r.get("weather_wind", "") or "")}</td></tr>'
            )
        parts.append("</table>")
    details = content.get("compliance_details", [])
    if details:
        parts.append(
            "<h2>Compliance Detail Records</h2><table><tr><th>Date</th><th>Product</th><th>EPA Reg#</th><th>Applicator</th><th>License</th><th>Target Pest</th><th>REI (hrs)</th></tr>"
        )
        for d in details:
            parts.append(
                f'<tr><td>{_html_escape(d.get("date", ""))}</td><td>{_html_escape(d.get("product_name", ""))}</td><td>{_html_escape(d.get("epa_reg_number", ""))}</td><td>{_html_escape(d.get("applicator_name", ""))}</td><td>{_html_escape(d.get("applicator_license", ""))}</td><td>{_html_escape(d.get("target_pest", ""))}</td><td>{d.get("rei_hours", "")}</td></tr>'
            )
        parts.append("</table>")
    if rei_issues:
        parts.append(
            '<h2 class="warning">REI Compliance Issues</h2><table><tr><th>Date</th><th>Product</th><th>REI (hrs)</th><th>Issue</th></tr>'
        )
        for i in rei_issues:
            parts.append(
                f'<tr><td>{_html_escape(i.get("date", ""))}</td><td>{_html_escape(i.get("product", ""))}</td><td>{i.get("rei_hours", "")}</td><td class="warning">{_html_escape(i.get("issue", ""))}</td></tr>'
            )
        parts.append("</table>")
    return parts
    ...
    for app in applications:
        ...
        writer.writerow(
            [
                ...
                app["rate"],
                app["rate_unit"],
                app.get("area_acreage", ""),
                app.get("total_product", ""),
                app.get("total_product_unit", ""),
                app.get("carrier_volume_gpa", ""),
                app.get("total_carrier_gallons", ""),
                app.get("weather_temp", ""),
                app.get("weather_wind", ""),
                app.get("weather_conditions", ""),
                app.get("application_method", ""),
                app.get("notes", ""),
                round(n_total, 4) if n_total else "",
                round(p_total, 4) if p_total else "",
                round(k_total, 4) if k_total else "",
            ]
        )
    ...
    for cr in records:
        writer.writerow(
            [
                cr.get("date", ""),
                cr.get("record_type", ""),
                cr.get("applicator_name", ""),
                cr.get("applicator_license", ""),
                cr.get("product_name", ""),
                cr.get("epa_reg_number", ""),
                cr.get("target_pest", ""),
                cr.get("area_treated", ""),
                cr.get("area_sqft", ""),
                cr.get("rate_applied", ""),
...
STATE_REQUIREMENTS = {
    "california": {
        ...
        "agency": "CA Department of Pesticide Regulation (DPR)",
        "report_name": "Pesticide Use Report (PUR)",
        ...
        "notes": "California has the most comprehensive pesticide reporting requirements in the US. All agricultural and structural pest control applications must be reported monthly. Golf courses and turf management fall under agricultural use reporting.",
    },
    "florida": {
        "state_name": "Florida",
        "agency": "FL Department of Agriculture and Consumer Services (FDACS)",
        "report_name": "Restricted-Use Pesticide Records",
        "frequency": "Records maintained for 2 years; available on request",
        "required_fields": [
            "date_of_application",
            "applicator_name",
            "applicator_license_number",
        ...
        "notes": "Florida requires records for all restricted-use pesticide applications. General-use pesticide records are recommended but not mandated. Records must be kept for a minimum of 2 years.",
    },
    "new york": {
        "state_name": "New York",
        "agency": "NY Department of Environmental Conservation (DEC)",
        "report_name": "Pesticide Application Report",
        "frequency": "Annual (due February 1)",
        "required_fields": [
            "date_of_application",
            "applicator_name",
        ...
        "notes": "New York requires annual reporting of all commercial pesticide applications. Reports are due by February 1 for the previous year. All certified applicators must report, including those applying to turf and ornamental sites.",
    },
    "texas": {
        "state_name": "Texas",
        "agency": "TX Department of Agriculture (TDA)",
        ...
        "notes": "Texas requires licensed applicators to maintain records of all pesticide applications for at least 2 years. Structural and non-structural applicators have different license categories. Records must be available for TDA inspection.",
    },
    "illinois": {
        "state_name": "Illinois",
        "agency": "IL Department of Agriculture",
        "report_name": "Pesticide Application Records",
        "frequency": "Records maintained for 3 years",
        "required_fields": [
            "date_of_application",
            "applicator_name",
            "applicator_license_number",
        ...
        "notes": "Illinois requires pesticide application records to be maintained for a minimum of 3 years. Both restricted-use and general-use application records are required for commercial applicators.",
    },
    "new jersey": {
        "state_name": "New Jersey",
        "agency": "NJ Department of Environmental Protection (DEP)",
        "report_name": "Pesticide Application Records",
        "frequency": "Records maintained for 3 years; restricted-use reported annually",
        "required_fields": [
            "date_of_application",
            "time_of_application",
            "applicator_name",
            ...
            "wind_speed_mph",
            "temperature_f",
            "notification_provided",
        ],
        "notes": "New Jersey has stringent notification requirements for pesticide applications on turf, particularly on school grounds and near sensitive areas. Pre-application notification may be required.",
    },
    "massachusetts": {
        "state_name": "Massachusetts",
        "agency": "MA Department of Agricultural Resources (DAR)",
        "report_name": "Pesticide Use Records",
        "frequency": "Records maintained for 3 years",
        "required_fields": [
            "date_of_application",
            "applicator_name",
        ...
        "notes": "Massachusetts requires all commercial applicators to maintain detailed records. Applications near water bodies require additional compliance with the Clean Waters Act.",
    },
    "georgia": {
        "state_name": "Georgia",
        "agency": "GA Department of Agriculture",
        "report_name": "Pesticide Application Records",
        "frequency": "Records maintained for 2 years",
        "required_fields": [
            "date_of_application",
            "applicator_name",
            "applicator_license_number",
        ...
        "notes": "Georgia requires licensed applicators to maintain records for at least 2 years. Both restricted-use and general-use pesticide applications should be recorded for commercial operations.",
    },
    "north carolina": {
        ...
        "notes": "North Carolina requires pesticide application records to be kept for 3 years. The state has additional buffer zone requirements near water bodies.",
    },
    "ohio": {
        "state_name": "Ohio",
        "agency": "OH Department of Agriculture",
        "report_name": "Pesticide Application Records",
        "frequency": "Records maintained for 3 years",
        "required_fields": [
            "date_of_application",
        ...
        "notes": "Ohio requires commercial applicators to maintain records for 3 years. Fertilizer applications on turf may also require separate nutrient management records.",
    },
    "michigan": {
        "state_name": "Michigan",
        "agency": "MI Department of Agriculture and Rural Development (MDARD)",
        "report_name": "Pesticide Application Records",
        "frequency": "Records maintained for 3 years",
        "required_fields": [
            "date_of_application",
        ...
        "notes": "Michigan requires detailed records for all commercial pesticide applications with a 3-year retention period. Additional rules apply to applications near Great Lakes waterways.",
    },
    "pennsylvania": {
        ...
        "notes": "Pennsylvania requires all certified applicators to maintain records for 3 years. The state follows EPA guidelines for record-keeping with additional state-specific requirements.",
    },
    "virginia": {
        "state_name": "Virginia",
        "agency": "VA Department of Agriculture and Consumer Services (VDACS)",
        "report_name": "Pesticide Application Records",
        "frequency": "Records maintained for 2 years",
        "required_fields": [
            "date_of_application",
        ...
        "notes": "Virginia requires pesticide applicator records for 2 years. Applications in the Chesapeake Bay watershed may have additional nutrient management and buffer zone requirements.",
    },
    "arizona": {
        "state_name": "Arizona",
        "agency": "AZ Department of Agriculture",
        "report_name": "Pesticide Application Records",
        "frequency": "Records maintained for 2 years",
        "required_fields": [
            "date_of_application",
        ...
        "notes": "Arizona requires licensed applicators to maintain records for 2 years. Additional water use reporting may be required for golf courses in water-restricted areas.",
    },
    "colorado": {
        "state_name": "Colorado",
        "agency": "CO Department of Agriculture",
        "report_name": "Pesticide Application Records",
        "frequency": "Records maintained for 3 years",
        "required_fields": [
            "date_of_application",
        ...
        "notes": "Colorado requires commercial applicators to keep records for 3 years. High-altitude turf management may require adjusted application rates that should be documented.",
    },
    "south carolina": {
        ...
        "notes": "South Carolina regulates pesticide use through Clemson University Department of Pesticide Regulation. Records must be maintained for 2 years for all commercial applications.",
    },
    "minnesota": {
        "state_name": "Minnesota",
        "agency": "MN Department of Agriculture",
        "report_name": "Pesticide Application Records",
        "frequency": "Records maintained for 5 years",
        "required_fields": [
            "date_of_application",
            "applicator_name",
            "applicator_license_number",
        ...
        "notes": "Minnesota has one of the longest record retention requirements at 5 years. The state also has specific pollinator protection requirements that may affect application timing.",
    },
    "connecticut": {
        "state_name": "Connecticut",
        "agency": "CT Department of Energy and Environmental Protection (DEEP)",
        "report_name": "Pesticide Application Records",
        "frequency": "Records maintained for 5 years; annual reporting required",
        "required_fields": [
            "date_of_application",
            "time_of_application",
            "applicator_name",
        ...
        "notes": "Connecticut requires 5-year record retention and annual reporting. The state has strong notification requirements and restrictions on pesticide use near schools and day care centers.",
    },
}
...
def delete_compliance_record(record_id, user_id):
    ...
    with get_db() as conn:
        conn.execute("DELETE FROM compliance_records WHERE id = ? AND user_id = ?", (record_id, user_id))
    return {"deleted": True}
