...
from db import FEEDBACK_DB, add_column, get_db
...
def get_negative_feedback(limit=50, unreviewed_only=True):
    ...
    for row in results:
        feedback_items.append(
            {"id": row[0], "question": row[1], "ai_answer": row[2], "user_correction": row[3], "timestamp": row[4]}
        )
...
def get_training_examples(unused_only=True, limit=1000):
    ...
    examples = []
    for row in results:
        examples.append({"id": row[0], "question": row[1], "ideal_answer": row[2]})
...
def get_feedback_stats():
    ...
    with get_db(FEEDBACK_DB) as conn:
        ...
        cursor = conn.execute(
            "SELECT COUNT(*) FROM feedback WHERE confidence_score < 60 AND confidence_score IS NOT NULL"
        )
        ...
        cursor = conn.execute(
            "SELECT COUNT(*) FROM feedback WHERE confidence_score < 70 AND confidence_score IS NOT NULL"
        )
        ...
        stats["confidence_trend"] = [
            {"date": row[0], "avg_confidence": round(row[1], 1) if row[1] else None} for row in cursor.fetchall()
        ]
...
def export_training_examples_csv():
    ...
    writer.writerow(["ID", "Question", "Ideal Answer", "Created At", "Used in Training", "Original Confidence"])
...
def export_moderation_history_csv():
    ...
    writer.writerow(["ID", "Feedback ID", "Action", "Moderator", "Reason", "Timestamp", "Question"])
...
if __name__ == "__main__":
    ...
    if negative:
        ...
        # Approve it
        approve_for_training(negative[0]["id"], negative[0]["user_correction"])
        print("\nApproved for training")
