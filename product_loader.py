...
def _load_pesticides():
    ...
    products = {}
    category_map = {"fungicides": "fungicide", "herbicides": "herbicide", "insecticides": "insecticide", "pgrs": "pgr"}
...
def _detect_form_type(info):
    ...
    if formulations:
        ...
        has_liquid = any(k in form_keys for k in ("sc", "ec", "tl", "se", "sl", "wg", "liquid", "emulsion_in_water"))
...
def get_all_products(user_id=None):
    ...
    """
    return sorted(_get_all_products_dict(user_id).values(), key=lambda p: (p["category"], p["display_name"]))
...
def search_products(query, user_id=None, category=None, form_type=None, inventory_only=False):
    ...
    for product in pool.values():
        ...
        if score is None:
            ...
            if all(any(sw.startswith(qw) for sw in searchable_words) for qw in query_words):
...
def add_to_inventory(user_id, product_id):
    ...
    with get_db() as conn:
        cursor = conn.execute(
            "INSERT OR IGNORE INTO user_inventory (user_id, product_id) VALUES (?, ?)", (user_id, product_id)
        )
...
def remove_from_inventory(user_id, product_id):
    ...
    with get_db() as conn:
        cursor = conn.execute("DELETE FROM user_inventory WHERE user_id = ? AND product_id = ?", (user_id, product_id))
