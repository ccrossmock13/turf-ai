...
class PromptVersioning:
    ...
    @staticmethod
    def create_version(template_text: str, description: str = "", changes: str = "", created_by: str = "admin") -> int:
        ...
        try:
            ...
            conn.commit()
            conn.close()
            log_event("prompt_versioning", "version_created", json.dumps({"id": version_id, "version": version}))
            return version_id
    ...
    def activate_version(version_id: int) -> bool:
        ...
        try:
            conn = _get_conn()
            conn.execute(
                "UPDATE prompt_templates SET is_active = 0, deactivated_at = CURRENT_TIMESTAMP WHERE is_active = 1"
            )
            conn.execute(
                """
            ...
            conn.commit()
            conn.close()
            log_event("prompt_versioning", "version_activated", json.dumps({"id": version_id}))
            return True
        except Exception as e:
    ...
    def get_active_version() -> Optional[Dict]:
        ...
        try:
            conn = _get_conn()
            row = conn.execute("SELECT * FROM prompt_templates WHERE is_active = 1 LIMIT 1").fetchone()
            conn.close()
    ...
    @staticmethod
    def log_usage(version_id: int, query_id: int, confidence: float = None, satisfaction: str = None):
        ...
        try:
            ...
            return {
                "v1": {
                    "id": v1_id,
                    "version": v1["version"],
                    "queries": v1["total_queries"],
                    "avg_confidence": v1["avg_confidence"],
                },
                "v2": {
                    "id": v2_id,
                    "version": v2["version"],
                    "queries": v2["total_queries"],
