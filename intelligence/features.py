...
class FeatureFlags:
    ...
    def set_flag(flag_name: str, enabled: bool, updated_by: str = "admin") -> bool:
        ...
        try:
            ...
            log_event(
                "feature_flags", "flag_toggled", json.dumps({"flag": flag_name, "enabled": enabled, "by": updated_by})
            )
            return True
        except Exception as e:
...
class RateLimiter:
    ...
    def _get_bucket(key: str, rate_per_min: int) -> dict:
        ...
        with RateLimiter._lock:
            if key not in RateLimiter._buckets:
                RateLimiter._buckets[key] = {"tokens": rate_per_min, "last_refill": now, "rate": rate_per_min}
    ...
    def _consume_token(key: str) -> bool:
        ...
        if r:
            try:
                r.hincrbyfloat(f"rl:{key}", "tokens", -1)
                return True
            except Exception:
                pass
        # In-memory fallback
    ...
    def cleanup_old_buckets():
        ...
        with RateLimiter._lock:
            to_delete = [k for k, v in RateLimiter._buckets.items() if v["last_refill"] < cutoff]
            for k in to_delete:
...
class InputSanitizer:
    ...
    def check_query(query: str, ip_address: str = None) -> Dict:
        ...
        return {"safe": action != "blocked", "score": total_score, "patterns": matched_patterns, "action": action}
