...
class AnomalyDetector:
    ...
    def record_metric(metric: str, value: float):
        ...
        AnomalyDetector._metric_history[metric].append({"value": value, "timestamp": time.time()})
        # Trim to max history
        if len(AnomalyDetector._metric_history[metric]) > AnomalyDetector._max_history:
            AnomalyDetector._metric_history[metric] = AnomalyDetector._metric_history[metric][
                -AnomalyDetector._max_history :
            ]
    ...
    def check_zscore(metric: str, value: float) -> Optional[Dict]:
        ...
        try:
            ...
            baseline = conn.execute(
                "SELECT mean, std, sample_count FROM metric_baselines WHERE metric = ?", (metric,)
            ).fetchone()
            conn.close()
    ...
    def check_all() -> List[Dict]:
        ...
        try:
            ...
            if detections:
                ...
                conn.commit()
                conn.close()
                log_event("anomaly_detector", "anomalies_detected", json.dumps({"count": len(detections)}), "warning")
