...
def process_answer_intelligence(
    query_id: int,
    question: str,
    answer: str,
    confidence: float,
    ...
    try:
        PipelineAnalytics.record_request(query_id=query_id, timings=timings, token_usage=token_usage)
        if token_usage:
    ...
    try:
        ...
        if embedding:
            ...
            if topic:
                result["topic"] = topic
                TopicIntelligence.store_question_embedding(question, embedding, query_id, topic["cluster_id"])
            else:
...
def process_feedback_intelligence(
    query_id: int,
    question: str,
    rating: str,
    confidence: float,
    ...
    try:
        ...
        ConfidenceCalibration.record_calibration_point(
            query_id=query_id, predicted_confidence=confidence, actual_rating=rating, topic=category
        )
        ...
        if sources:
            for source in sources:
                ...
                SourceQualityIntelligence.update_source_reliability(
                    source_id=source_id, rating=rating, confidence=confidence, source_title=source_title
                )
        ...
        if correction and len(correction.strip()) > 20:
            _auto_promote_correction_to_golden(
                query_id=query_id, question=question, correction=correction, category=category
            )
...
def _auto_promote_correction_to_golden(query_id: int, question: str, correction: str, category: str = None):
    ...
    try:
        ...
        if existing:
            ...
            log_event(
                "self_healing",
                "golden_answer_updated_from_correction",
...
def _track_negative_accumulation(question: str, category: str, query_id: int):
    ...
    try:
        ...
        if neg_count >= 3:
            log_event(
                "feedback_loop",
                "negative_accumulation_alert",
                json.dumps({"category": category, "negative_count_7d": neg_count, "latest_query_id": query_id}),
                "warning",
            )
    except Exception as e:
