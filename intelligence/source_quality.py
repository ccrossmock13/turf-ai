...
class SourceQualityIntelligence:
    ...
    def update_source_reliability(
        source_id: str, rating: str, confidence: float = None, source_title: str = None, source_type: str = None
    ):
        """Update reliability score for a source based on user feedback."""
        is_positive = rating in ("helpful", "good", "correct")

        conn = _get_conn()
        existing = conn.execute("SELECT * FROM source_reliability WHERE source_id = ?", (source_id,)).fetchone()
        ...
        if existing:
            ...
            conn.execute(
                ...
                WHERE source_id = ?
            """,
                (trust, pos, neg, total, new_avg, source_title, source_type, datetime.now().isoformat(), source_id),
            )
        else:
    ...
    def get_source_reliability(source_id: str) -> Optional[Dict]:
        ...
        conn = _get_conn()
        row = conn.execute("SELECT * FROM source_reliability WHERE source_id = ?", (source_id,)).fetchone()
        conn.close()
    ...
    def apply_source_adjustments(sources: List[Dict]) -> List[Dict]:
        ...
        for source in sources:
            ...
            reliability = conn.execute(
                "SELECT trust_score, admin_boost FROM source_reliability WHERE source_id = ?", (source_id,)
            ).fetchone()
    ...
    def set_admin_boost(source_id: str, boost: float):
        ...
        conn.commit()
        conn.close()
        log_event("source_quality", "admin_boost_set", json.dumps({"source_id": source_id, "boost": boost}))
