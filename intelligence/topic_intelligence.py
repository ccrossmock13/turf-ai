...
from intelligence.helpers import _agglomerative_cluster, _auto_name_cluster, _compute_centroid, _cosine_similarity
...
class TopicIntelligence:
    ...
    @staticmethod
    def store_question_embedding(question: str, embedding: List[float], query_id: int = None, cluster_id: int = None):
    ...
    def cluster_questions(min_cluster_size: int = 5, similarity_threshold: float = 0.7) -> Dict:
        ...
        for _cluster_idx, member_indices in clusters.items():
        ...
        for cluster in clusters:
            ...
            if rows:
                ...
                conn.execute(
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """,
                    (cluster["id"], period_start, period_end, len(rows), avg_conf, avg_sat, negative, positive),
                )
                ...
                conn.execute(
                    ...
                    WHERE id = ?
                """,
                    (len(rows), avg_conf, avg_sat, neg_rate, period_end, cluster["id"]),
                )
