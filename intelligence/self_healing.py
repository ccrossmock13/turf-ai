...
class SelfHealingLoop:
    ...
    def detect_weak_patterns(min_occurrences: int = 3, days: int = 30) -> List[Dict]:
        ...
        if patterns:
            log_event("self_healing", "weak_patterns_detected", json.dumps({"count": len(patterns)}))
        return patterns

    @staticmethod
    def create_golden_answer(
        question: str, answer: str, category: str = None, source_feedback_id: int = None, embedding: List[float] = None
    ) -> int:
        ...
        log_event("self_healing", "golden_answer_created", json.dumps({"id": golden_id, "category": category}))
        return golden_id

    @staticmethod
    def get_relevant_golden_answers(
        query: str, category: str = None, query_embedding: List[float] = None, limit: int = 3
    ) -> List[Dict]:
        """
    ...
    def record_golden_answer_usage(golden_id: int, rating: str = None):
        ...
        if rating:
            ...
            row = conn.execute(
                "SELECT times_used, avg_rating_when_used FROM golden_answers WHERE id = ?", (golden_id,)
            ).fetchone()
            if row:
                ...
                conn.execute("UPDATE golden_answers SET avg_rating_when_used = ? WHERE id = ?", (new_avg, golden_id))
