...
class CircuitBreaker:
    ...
    def record_failure(source_id: str):
        ...
        try:
            ...
            existing = conn.execute("SELECT * FROM circuit_breakers WHERE source_id = ?", (source_id,)).fetchone()
                ...
                if last and (now - last).total_seconds() > window:
                    ...
                    if new_count >= threshold:
                        ...
                        conn.execute(
                            ...
                            WHERE source_id = ?
                        """,
                            (new_count, now.isoformat(), now.isoformat(), recovery_at, now.isoformat(), source_id),
                        )
                        log_event(
                            "circuit_breaker",
                            "opened",
                            json.dumps({"source_id": source_id, "failures": new_count}),
                            "warning",
                        )
                    else:
                        conn.execute(
    ...
    def is_open(source_id: str) -> bool:
        ...
        try:
            conn = _get_conn()
            row = conn.execute(
                "SELECT state, recovery_at FROM circuit_breakers WHERE source_id = ?", (source_id,)
            ).fetchone()
            conn.close()
    ...
    def _close(source_id: str):
        ...
        try:
            ...
            conn.commit()
            conn.close()
            log_event("circuit_breaker", "recovered", json.dumps({"source_id": source_id}))
        except Exception as e:
