...
class AlertEngine:
    ...
    @staticmethod
    def create_rule(
        name: str, metric: str, condition: str, threshold: float, channels: List[str] = None, cooldown_minutes: int = 60
    ) -> int:
        ...
        try:
            ...
            cursor = conn.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            """,
                (name, metric, condition, threshold, json.dumps(channels or ["in_app"]), cooldown_minutes),
            )
    ...
    def evaluate_rules() -> List[Dict]:
        ...
        try:
            ...
            for rule in rules:
                ...
                if triggered:
                    message = (
                        f"Alert: {rule['name']} — {rule['metric']} is "
                    ...
                    for channel in channels:
                        AlertEngine._dispatch(
                            rule["id"], rule["name"], rule["metric"], metric_val, rule["threshold"], channel, message
                        )
            ...
            if fired:
                log_event("alert_engine", "rules_fired", json.dumps({"count": len(fired)}), "warning")
    ...
    def _collect_current_metrics(conn) -> Dict[str, float]:
        ...
        try:
            ...
            if lats:
                n = len(lats)
                metrics["latency_p95"] = (
                    lats[int(n * 0.95)]["total_latency_ms"] if n > 1 else lats[0]["total_latency_ms"]
                )
    ...
    @staticmethod
    def _dispatch(
        rule_id: int, rule_name: str, metric: str, value: float, threshold: float, channel: str, message: str
    ):
