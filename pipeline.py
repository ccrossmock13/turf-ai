...
from constants import DEFAULT_SOURCES, MAX_CONTEXT_LENGTH, MAX_SOURCES, SEARCH_FOLDERS
...
from query_expansion import expand_query, expand_vague_question, get_query_intent
from query_rewriter import rewrite_query
from reranker import is_cross_encoder_available, rerank_results
...
from turf_intelligence import (
    ...
    build_cost_context,
    build_cross_module_context,
    build_diagnostic_context,
    build_frac_rotation_context,
    build_seasonal_context,
    build_weather_spray_context,
    generate_follow_up_suggestions,
    generate_predictive_alerts,
    get_cultivar_context,
    log_knowledge_gap,
    select_model,
)
from weather_service import format_weather_for_response, get_weather_context, get_weather_data, get_weather_warnings
from web_search import (
    format_web_search_disclaimer,
    search_web_for_turf_info,
    should_supplement_with_web_search,
...
def is_significant_topic_change(previous_topic, current_topic, question, previous_subject=None, current_subject=None):
...
def build_messages_with_history(conversation_id, system_prompt, context, current_question):
    ...
    messages.append({"role": "user", "content": build_user_prompt(context, current_question)})
    return messages
...
def run_pre_llm_pipeline(ctx: PipelineContext, body: dict) -> Optional[dict]:
    ...
    """
    ctx.trace = Trace(question=ctx.question, user_id=ctx.user_id, session_id=ctx.session_data.get("session_id"))
    ...
    ctx.trace.step(
        "detect", topic=ctx.question_topic, grass=ctx.grass_type, region=ctx.region, product_need=ctx.product_need
    )
    ...
    try:
        ...
        search_results = {"general": {"matches": []}, "product": {"matches": []}, "timing": {"matches": []}}
        try:
    ...
    ctx.trace.step("score_rerank", scored_count=len(scored_results), cross_encoder=is_cross_encoder_available())
    ...
    if should_trigger_web_search(search_results) or ctx.pinecone_failed:
        reformulated = _try_query_reformulation(ctx, search_results, scored_results, filtered_results, context, sources)
        if reformulated:
    ...
    from prompts import build_reference_context, build_system_prompt
    ...
    try:
        ...
        if ctx.question_topic == "diagnostic" or (
            hasattr(ctx, "question")
            and any(kw in ctx.question.lower() for kw in ["diagnose", "identify", "what's wrong", "what is wrong"])
        ):
    ...
    # Golden answers
    try:
        golden_answers = SelfHealingLoop.get_relevant_golden_answers(query=ctx.question, category=ctx.question_topic)
        if golden_answers:
    ...
    if ctx.is_topic_change:
        ...
        ctx.messages = build_messages_with_history(ctx.conversation_id, ctx.system_prompt, ctx.context, ctx.question)

    ctx.timings["6_pre_llm"] = time.time() - ctx.t0
    ctx.trace.step(
        "pre_llm", context_len=len(ctx.context), source_count=len(ctx.sources), web_search=ctx.used_web_search
    )
    ...
    try:
        ...
        if budget_action == "budget_exceeded":
            golden_hit = SelfHealingLoop.get_relevant_golden_answers(query=ctx.question, category=ctx.question_topic)
            if golden_hit:
                return _build_final_response(
                    ctx, golden_hit[0]["answer"], {"prompt_tokens": 0, "completion_tokens": 0, "model": "cached"}
                )
            return {
...
def _build_final_response(ctx: PipelineContext, assistant_response: str, token_usage: dict) -> dict:
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        grounding_future = executor.submit(
            check_answer_grounding, ctx.openai_client, assistant_response, ctx.context, ctx.question, "gpt-4o-mini"
        )
    ...
    needs_review = (
        confidence < 70
        or not grounding_result.get("grounded", True)
        or len(grounding_result.get("unsupported_claims", [])) > 1
        or not ctx.sources
    )
    ...
    try:
        ...
        follow_ups = generate_follow_up_suggestions(
            ctx.question, assistant_response, intent=_intent, disease=ctx.current_subject, product=ctx.product_need
        )
    except Exception:
        pass
    ...
    try:
        gap_result = assess_knowledge_gaps(ctx.sources, confidence, ctx.question, context=ctx.context)
    ...
    try:
        ...
        alerts = generate_predictive_alerts(
            ctx.user_id, weather_data=ctx.weather_data, state=_user_state, grass_type=ctx.grass_type
        )
    except Exception:
        pass
    ...
    ctx.trace.finish(
        confidence=confidence,
        source_count=len(ctx.display_sources),
