...
VALID_ROLES = ["superintendent", "assistant_super", "spray_tech", "mechanic", "crew_leader", "crew_member", "intern"]

VALID_AREAS = ["greens", "fairways", "tees", "rough", "bunkers", "practice", "clubhouse", "all"]
...
def delete_crew_member_permanent(member_id, user_id):
    ...
    try:
        with get_db() as conn:
            row = conn.execute(
                "SELECT id FROM crew_members WHERE id = ? AND user_id = ?", (member_id, user_id)
            ).fetchone()
            if not row:
                return {"error": "Crew member not found or access denied"}
            ...
            conn.execute(
                "UPDATE work_orders SET assigned_to = NULL WHERE assigned_to = ? AND user_id = ?", (member_id, user_id)
            )
...
def assign_work_order(wo_id, user_id, crew_member_id):
    ...
    try:
        ...
        logger.info(f"Assigned work order {wo_id} to crew member " f"{crew_member_id} for user {user_id}")
...
def get_work_orders(user_id, status=None, area=None, assigned_to=None, date_range=None):
...
def log_time(user_id, data):
    ...
    try:
        ...
        logger.info(f"Logged {hours}h for crew member {crew_member_id} " f"on {entry_date} (entry={entry_id})")
        return {"id": entry_id}
    except Exception as e:
...
def get_time_entries(user_id, crew_member_id=None, start_date=None, end_date=None):
...
def get_labor_summary(user_id, start_date, end_date):
    ...
    try:
        ...
        by_area = [
            {"area": r["area"], "hours": round(r["total_hours"], 2), "entries": r["entry_count"]} for r in area_rows
        ]
        by_task_type = [
            {"task_type": r["task_type"], "hours": round(r["total_hours"], 2), "entries": r["entry_count"]}
            for r in task_rows
        ]
        by_crew_member = [
            {"id": r["id"], "name": r["name"], "hours": round(r["total_hours"], 2), "entries": r["entry_count"]}
...
def get_labor_cost(user_id, start_date, end_date):
    ...
    try:
        ...
        by_area = [
            {"area": r["area"], "hours": round(r["total_hours"] or 0, 2), "cost": round(r["total_cost"] or 0, 2)}
            for r in area_rows
        ]
...
def create_daily_assignments(user_id, date, assignments):
    ...
    try:
        with get_db() as conn:
            ...
            member_ids = set(a.get("crew_member_id") for a in assignments if a.get("crew_member_id"))
            if member_ids:
            ...
            for i, assignment in enumerate(assignments):
                ...
                if cm_id not in valid_ids:
                    errors.append(f"Assignment {i}: crew member {cm_id} not found or access denied")
                    continue
                if not task_desc:
        ...
        logger.info(f"Created {len(created_ids)} daily assignments for " f"user {user_id} on {date}")
        return result
    except Exception as e:
...
def get_productivity_report(user_id, start_date, end_date):
    ...
    try:
        ...
        hours_by_area = [
            {
                ...
                "avg_hours_per_day": round(r["total_hours"] / max(r["days_worked"], 1), 2),
            }
            for r in area_rows
        ]
        ...
        daily_breakdown = [
            {"date": r["date"], "hours": round(r["total_hours"], 2), "crew_count": r["crew_count"]} for r in daily_rows
        ]
        return {
...
def get_crew_utilization(user_id, start_date, end_date):
    ...
    try:
        ...
        overall = round((total_worked / total_available * 100), 1) if total_available > 0 else 0.0
...
def get_overtime_report(user_id, start_date, end_date):
    ...
    try:
        ...
        for r in rows:
            ...
            member_weeks[mid]["weeks"][week_key] = member_weeks[mid]["weeks"].get(week_key, 0.0) + (r["hours"] or 0.0)
        # Calculate overtime
        by_member = []
...
def update_assignment(assignment_id, user_id, data):
    ...
    with get_db() as conn:
        assignment = conn.execute(
            "SELECT id FROM daily_assignments WHERE id = ? AND user_id = ?", (assignment_id, user_id)
        ).fetchone()
        if not assignment:
        ...
        if sets:
            ...
            conn.execute(f'UPDATE daily_assignments SET {", ".join(sets)} WHERE id = ? AND user_id = ?', vals)
...
def delete_assignment(assignment_id, user_id):
    ...
    with get_db() as conn:
        conn.execute("DELETE FROM daily_assignments WHERE id = ? AND user_id = ?", (assignment_id, user_id))
    return {"deleted": True}
