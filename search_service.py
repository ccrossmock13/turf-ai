...
from cache import get_cached_embedding, get_cached_search_results, get_cached_source_url
...
def detect_specific_subject(question_lower):
    ...
    # Grass types
    grass_types = ["bermuda", "bentgrass", "zoysia", "bluegrass", "fescue", "ryegrass"]
...
def search_general(index, embedding, top_k=GENERAL_SEARCH_TOP_K, query_text=""):
    ...
    try:
        results = get_cached_search_results(index, embedding, "general", top_k, filters=None, query_text=query_text)
...
def search_products(index, openai_client, question, product_need, model="text-embedding-3-small"):
    ...
    if any(word in question_lower for word in TOPIC_KEYWORDS["algae"]):
        ...
        return get_cached_search_results(
            index, embedding, "algae", ALGAE_SEARCH_TOP_K, filters=None, query_text=algae_query
        )
    ...
    if any(word in question_lower for word in TOPIC_KEYWORDS["product"]):
        ...
        results = get_cached_search_results(
            index, embedding, "product", PRODUCT_SEARCH_TOP_K, filters=search_filter, query_text=product_query
        )
...
def search_timing(index, openai_client, question, grass_type, model="text-embedding-3-small"):
    ...
    if any(word in question_lower for word in TOPIC_KEYWORDS["timing"]):
        ...
        return get_cached_search_results(
            index, embedding, "timing", TIMING_SEARCH_TOP_K, filters=None, query_text=timing_query
        )
...
def search_all_parallel(
    index, openai_client, question, expanded_query, product_need, grass_type, model="text-embedding-3-small"
):
    """
    ...
    """
    results = {"general": {"matches": []}, "product": {"matches": []}, "timing": {"matches": []}}
