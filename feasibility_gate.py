...
PARAMETER_BOUNDS = {
    ...
    "nitrogen_per_1000": {
        "min": 0.01,
        "max": 8.0,
        "label": "Nitrogen per 1000 sq ft per application",
        "typical": "0.25-1.5 lbs",
    },
    "mowing_height_home": {"min": 0.5, "max": 6.0, "label": "Home lawn mowing height", "typical": "2.5-4.0 inches"},
    "mowing_height_green": {
        "min": 0.06,
        "max": 0.200,
        "label": "Putting green height",
        "typical": "0.100-0.150 inches",
    },
...
def _check_conflicting_actions(q: str) -> List[Dict]:
    ...
    has_seeding = any(
        term in q for term in ["overseed", "over-seed", "seed", "seeding", "reseed", "establish from seed"]
    )
...
def _check_absurd_parameters(q: str, original: str) -> List[Dict]:
    ...
    n_match = re.search(
        r"(\d+\.?\d*)\s*(?:lbs?|pounds?)\s*(?:of\s+)?(?:nitrogen|n)\s*(?:per|/)\s*(?:1[,.]?000|thousand)", q
    )
    if not n_match:
    ...
    if mow_match:
        ...
        any(term in q for term in ["green", "putting"])
...
def _check_safety_violations(q: str, original: str) -> List[Dict]:
    ...
    double_rate = any(
        ...
        for phrase in [
            "double the rate",
            "double the fungicide",
            "double the herbicide",
            "double the insecticide",
            "twice the rate",
            "twice the label",
    ...
    rei_violation = any(
        ...
        for phrase in [
            "play immediately",
            "golfers play immediately",
            "play right after",
            "play right away",
            "let golfers on",
            "mow right after spray",
            "no waiting",
            "no wait time",
        ]
    )
    mentions_spray_product = any(
        term in q
        for term in [
            "spray",
            "apply",
    ...
    illegal_disposal = any(
        ...
        for phrase in [
            ...
            "dump leftover",
            "pour leftover",
            "dispose in the ditch",
            "throw away pesticide",
            "pour out the extra",
            "dump the extra",
        ]
    )
    ...
    mixing_leftovers = any(
        phrase in q
        for phrase in [
            "mix leftover",
            "mix remaining",
            "combine leftover",
            "mix pesticides together",
            "mix different pesticides",
            "mix them together",
            "pour them together",
        ]
    )
...
def _build_feasibility_response(issues: List[Dict], question: str) -> Dict:
    ...
    if high_severity:
        parts.append(
            "I noticed some important concerns with your question that I want to address before providing advice:\n"
        )
